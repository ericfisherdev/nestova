...
func buildWebAuthnSettingsTestHandler(t *testing.T) (http.Handler, *scs.SessionManager, *fakeWebAuthnCredentialRepo, *multiMemberHouseholdRepo, *authapp.MFAService) {
	...
	registerSettingsPage(mux, logger, sm, hhRepo, settingsHandlers, mfaHandlers, mfaService, newTestPINHandlers(hhRepo, sm, logger), webauthnHandlers, webauthnService, newTestNotifyWebHandlers(hhRepo, sm, logger), config.PeerConfig{}, nil)
