...
Commands below reference two environment variables rather than hard-coded
values: `$SMS_ALERT_EMAIL` (the address used at setup time — set this in
your shell before running any command below) and `$SMS_REGION` (the Region
End User Messaging is configured in — see `docs/aws-sms.md`).

> **Guardrail status: incomplete (single-recipient).** Both budgets below
> currently notify only `$SMS_ALERT_EMAIL`, though this household has two
> parents. Treat this guardrail as NOT fully set up until the second
> parent's email is added — a single point of failure in the alerting path
> defeats the "warns someone" purpose of a budget alert.
>
> **TODO(second parent):** add the second parent's email to both budgets
> when known: re-run the `create-notification` commands with the extra
...
AWS Budgets' IAM action namespace is unusually coarse: unlike most AWS
services, there is no separate `budgets:CreateBudget` /
`budgets:CreateNotification` / `budgets:CreateSubscriber` /
`budgets:DeleteBudget` action — every mutating call this runbook makes
(`create-budget`, `create-notification`, `create-subscriber`,
`update-budget`, `delete-budget`) is authorized under the single
`budgets:ModifyBudget` action; `budgets:ViewBudget` covers the read-only
`describe-budgets` calls. (`budgets:CreateBudgetAction` /
`budgets:ModifyBudgetAction` are a distinct family for the separate
...
for spec in "ACTUAL 50" "FORECASTED 80" "ACTUAL 100"; do
  ...
  aws budgets create-notification --account-id 768962091675 \
    ...
    --subscribers "SubscriptionType=EMAIL,Address=$SMS_ALERT_EMAIL"
done
```
...
```bash
aws pinpoint-sms-voice-v2 describe-spend-limits --region "$SMS_REGION"   # check MaxLimit first
aws pinpoint-sms-voice-v2 set-text-message-spend-limit-override --region "$SMS_REGION" --monthly-limit 10
aws pinpoint-sms-voice-v2 describe-spend-limits --region "$SMS_REGION"
```
...
aws pinpoint-sms-voice-v2 describe-spend-limits --region "$SMS_REGION" --output table
```
