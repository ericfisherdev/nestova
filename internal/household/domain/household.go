...
// Household rows now live in the shared identity schema (NSTR-115,
// identity.household) — this struct mirrors exactly the columns nestova
// still cares about. Quiet hours, formerly fields on this struct, moved to
// internal/notify's own domain (notify's own household-keyed table) since
// notifications are a per-app concern the identity schema must not carry.
