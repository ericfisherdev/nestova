...
-- lets the sweep avoid a full-table scan.
CREATE INDEX task_instance_pending_due_idx ON task_instance (due_on) WHERE status = 'pending';

-- +goose Down
DROP INDEX IF EXISTS task_instance_pending_due_idx;
